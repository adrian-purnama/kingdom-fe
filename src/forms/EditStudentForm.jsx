...
/**
 * @param {object} props
 * @param {{ id: string; name: string; nim: string; point: number; housing: string }} props.student
 * @param {() => void} props.onSuccess
 * @param {() => void} props.onCancel
 */
export function EditStudentForm({ student, onSuccess, onCancel }) {
  const [name, setName] = useState(student.name ?? "");
  const [nim, setNim] = useState(student.nim ?? "");
  const [point, setPoint] = useState(String(student.point ?? 0));
  const [housing, setHousing] = useState(student.housing ?? "scientia");
  const [gender, setGender] = useState(student.gender ?? "");
  const [characterRole, setCharacterRole] = useState(student.characterRole ?? "");
  ...
  async function onSubmit(e) {
    ...
    setSubmitting(true);
    try {
      await apiPatch(`${paths.adminStudents}/${student.id}`, {
        name: n,
        nim: nimVal,
        point: pointNum,
        housing,
      ...
      setFormErr(err?.message ?? "Could not update student");
    } finally {
      setSubmitting(false);
    }
  }
